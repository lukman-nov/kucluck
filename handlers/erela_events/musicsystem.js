...
module.exports = client => {
  client.on("interactionCreate", async (interaction) => {
    ...
    const ss = await client.Settings.findOne({ GuildId: guild.id })
    const ls = ss.Language;
    const es = ss.Embed;
    const autoresume = await client.autoresume.findOne({ guild: guild.id })
    const premium = await client.Premium.findOne({ GuildId : guild.id });
    const musicsystem = await client.Musicsettings.findOne({ guildId : guild.id });
    ...
    if (interaction.isButton()) {
      ...
      switch (interaction.customId) {
        case "Skip": {
          ...
          if (player.queue.size === 0) {
            ...
            if (premium.BotAFK) {
              ...
              await client.autoresume.findOneAndDelete({ guild: guild.id });
              await player.stop();
        ...
        case "Stop": {
          //Stop the player
          if (premium && premium.BotAFK === true) {
            interaction.reply({
              embeds: [new MessageEmbed()
                .setColor(es.color)
                .setTimestamp()
            ...
            if (player) {
              // await player.destroy();
              if (autoresume) await client.autoresume.findOneAndDelete({ guild : guild.id })
  ...
  client.on("messageCreate", async message => {
    ...
    var data = await client.Musicsettings.findOne({ guildId : message.guild.id }).clone();
    if(!data) return;
    ...
    const ss = await client.Settings.findOne({ GuildId : message.guild.id });
