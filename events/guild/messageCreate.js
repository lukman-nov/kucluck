...
const { escapeRegex, delay, databasing, handlemsg, leveling, isMessageDeleted } = require(`${process.cwd()}/handlers/functions`);
...
module.exports = async (client, message) => {
  try {
    ...
    let stsG = await StastGSchema.findOne({ BotId : client.user.id }).clone();
    let sts = await StatsSchema.findOne({ GuildId : message.guild.id }).clone();
    let ss = await SettingsSchema.findOne({ GuildId : message.guild.id }).clone();
    let ms = await MusicSchema.findOne({ guildId : message.guild.id }).clone();
    databasing(client, message.guild.id, message.author.id);
    let premiumguild = await premiumGuildSchema.findOne({ GuildId: message.guild.id }).clone();
    ...
    if (premiumguild && !premiumguild.Permanent && Date.now() > premiumguild.Expire){
      ...
      guild.fetchOwner().then(owner => {
        owner.send(`Thanks for subscribe __**Kucluck Premium**__.\n\nNow **Kucluck Premium** has __**expired**__ in __**${message.guild.name}**__`).catch(() => {});
      }).catch(() => {});
      if (ss.AutoPlay) {
        ss.AutoPlay = false
        ss.save();
      ...
      if (command.premium) {
        ...
        if (!premiumguild) {
          ...
          return message.reply({
            embeds: [new MessageEmbed()
              .setColor(es.wrongcolor)
              .setTitle(`${emoji.msg.x} Your guild is not premium`)
              .setDescription(`**Let's a subscribe Kucluck Premium for use the Premium Commands.**`)
            ]
          });
        }
      }
