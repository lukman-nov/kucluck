          ...
          if (!premiumguild) {
            ...
            return interaction.reply({
              embeds: [new Discord.MessageEmbed()
                ...
                .setDescription(`Slash commands only work for Premium Guilds. Let's a subscribe Kucluck Premium for use the Premium Commands.`)
            ...
            if (!premiumguild.Permanent && Date.now() > premiumguild.Expire){
              ...
              guild.fetchOwner().then(owner => {
                owner.send(`❌ **Kucluck Premium has __expired!__ in your __${guild.name}__ server **`).catch(() => {});
              }).catch(() => {});
              
              return interaction.reply({
                embeds: [new Discord.MessageEmbed()
                  .setColor("#FFD700")
                  .setTitle("Kucluck Premium in this guild has __expired!__")
                  .setDescription(`Subscribe to Kucluck Premium again and get a 50% discount.`)
                  .setFooter(client.getFooter(es))
                  .setThumbnail(url.img.coin)
                ]
              });
