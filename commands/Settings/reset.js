...
const ReactRoleSchema = require('../../databases/reaction-roles');
const PremiumSchema = require('../../databases/premium');
const MuteSchema = require('../../databases/mutemember');
const MemberCountSchema = require('../../databases/memberCount');
...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
        ...
        async function handle_the_picks(menuoptionindex, menuoptiondata) {
            var autorole = await client.Autorolesettings.findOne({ GuildId : message.guild.id });
            var embed = await client.Embedsettings.findOne({ guildId : message.guild.id });
            var greetingmsg = await client.Greetingmsg.findOne({ GuildId : message.guild.id });
            var membercount = await client.Membercount.findOne({ GuildId : message.guild.id });
            var mute =  await client.Mutesettings.findOne({ GuildId : message.guild.id });
            var premium = await client.Premium.findOne({ GuildId : message.guild.id });
            var reactionrole = await client.reactrole.findOne({ GuildId: message.guild.id });
            var dbsettings = await client.Settings.findOne({ GuildId: message.guild.id });
            var stats =  await client.stats.findOne({ GuildId : message.guild.id});
            var musicsettings = await client.Musicsettings.findOne({ guildId : message.guild.id });
            var levelingS = await client.leveling.findOne({ GuildId : message.guild.id });
                        ...
                        try {
                          if (i.isButton()) {
                            ...
                            if (i.customId == "1") {
                                ...
                                if (reactionrole) reactionrole.delete();
                                if (dbsettings) dbsettings.delete();
